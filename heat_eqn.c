...
int main()
{
    ...
    printf("This program solves the heat equation \nu_t = alpha * u_xx with either Dirichlet or Neumann boundary conditions.\n");
    printf("Do you want to solve for Dirichlet (D) or Neumann (N) scheme: ");
    scanf("%s", scheme);
    printf("Enter constant alpha [default value 1] (integer): ");
    scanf("%d", &alpha);
    ...
    if (!strcmp(scheme, "D") || !strcmp(scheme, "d"))
        ...
        impheateqn_n(alpha, nx * DEFAULT_SPACE_STEP, nt * DEFAULT_TIME_STEP, dt);
